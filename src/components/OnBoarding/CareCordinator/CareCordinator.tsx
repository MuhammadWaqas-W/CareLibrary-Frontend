...
import { CarerCordinatorDetails, ICarerCordinatorDetails } from '../../../mock/OnBoarding';
...
const CarerCordinator = () => {
  ...
  const items: any = [
    ...
    },
    {
      label: (
        <Space >
          <img src={ClientDetails} alt="Delete" className="d-flex align-center" width={24} height={24} />
          <span onClick={() => navigate('client-details')}>Client Details</span>
        </Space>
      ),
      key: "3",
    },
      ...
      render: (_, text) =>
        <div>
          <div className='fs-14 fw-400 title-color' style={{ minWidth: "70px" }}>
            <span className='fw-700 fs-14'>{text?.percentage}%</span>   <span style={{ marginLeft: "5px" }} className='fw-600 fs-14'>Done</span>
          </div>
          <Progress percent={text?.percentage} strokeColor='#F7B923' strokeWidth={9} width={50} showInfo={false} />
        </div>
    },
  ...
  return <div className='carer-wrapper'>
    <Row className="carer-main d-flex justify-end carer-style" >
      <Col span={24} className="gutter-row" style={{ marginBottom: "20px" }}>
        <div className='candidate-wrapper'>
          <button onClick={() => setIsShowCordinator(true)} className="candidate-button fw-600 fs-16 cursor-pointer" >
            Add New Cordinator
          </button>
          <OnboardingAddModal isOpenModal={isShowCordinator} setIsOpenModal={setIsShowCordinator} title='Add New Cordinator' />
        </div>
      </Col>
      <Col xs={24}>
        <Row
    ...
    <Row className='carer-main'>
      <Col span={24} className='onboarding-table-wrapper' >
        ...
        <Table className="wrapper-table" columns={columns} dataSource={CarerCordinatorDetails} scroll={{ x: "max-content" }} pagination={{ pageSize: 7 }} />
      </Col>
    </Row>
