...
import { clientShiftCompletedData } from "../../../../mock/ClientManageShift";
import "./Modal.scss";
import TextArea from "antd/es/input/TextArea";
import ConfirmAndSignOffModal from "./ConfirmAndSignOffModal";

const CompletedConfirmModal = (props: any) => {
  const { isCompletedModifyModal, setIsCompletedModifyModal } = props;
      ...
      <Modal title="Shift Information" open={isCompletedModifyModal} onCancel={() => setIsCompletedModifyModal(false)} footer={false} className="confirm-modal-wrapper" width={850} centered>
        <div className="confirm-modal-content">
          ...
          <div className="shift-btn-wrapper d-flex align-center" style={{ paddingTop: "30px" }}>
            ...
            <button type="submit" className="confirm-btn fs-16 fw-600 white-color m-0 cursor-pointer" onClick={() => {setIsConfirmModalOpen(true); setIsCompletedModifyModal(false)}}>
              Confirm and Sign Off
            </button>
          </div>
        </div>
      </Modal>
      <ConfirmAndSignOffModal open={isConfirmModalOpen} onCancel={() => setIsConfirmModalOpen(false)} heading={'Are you sure you want to sign off this shift’s (Modified) Check in and Check out time'} CancelButton={() => setIsConfirmModalOpen(false)} />
    </>
  );
};
