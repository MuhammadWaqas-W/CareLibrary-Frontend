        ...
        {(role === ROLES.admin || role === ROLES.client || role === ROLES.coordinator )&&
            ...
            <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
              <Col className="gutter-row" xs={24} sm={24} md={24}lg={24} xxl={24} xl={24}>
                  <div className='faqs-tittle '>
                      <h5 className='fw-500 fs-30' style={{lineHeight:"40px"}}>How can we help you ?</h5>
                      <Input
                          placeholder="Search"
                          prefix={
                              <img src={Search} alt="search icon" className="icon"  style={{marginRight:"10px"}}/>
                          }
          ...
          <Row  gutter={[16, 16]}>
              ...
              {faqsData.map((faq: any) => {
                  return (
                      <Col className="gutter-row" sm={12} md={12} lg={8} xl={8} xxl={8}>
                          <div className='wrap-dev' style={{ marginBottom: "55px",margin:"0 auto",width:"70%" }}>

                              <ul className="admin-list">
                                  <p className='fw-600 fs-20 form-heading-color ' style={{ marginBottom: "15px" }}>{faq.title}</p>
                                  {faq.optionsArray.map((option: any) => {
                                      return (
                                          <li className='fw-400 fs-14 title-color line-height-22' key={option.key} style={{marginBottom:"5px"}}>{option.list}</li>
                                      )
                                  })}
                              </ul>
                          </div>
                    ...
                    return (
                            <Collapse accordion  bordered={false}>
                                ...
                                <ul>
                                    {faq.optionsArray.map((option: any) => {
                                        return (
                                            <li className='fw-400 fs-14 title-color' style={{marginBottom:"5px"}}>{option.list}</li>
                                        )
                                    })}
                                </ul>
                                </Panel>
